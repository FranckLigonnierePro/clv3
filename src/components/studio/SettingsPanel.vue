...
<template>
  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
    <!-- Panel -->
    <div class="bg-gray-800 border border-gray-700 rounded-2xl p-6 w-full max-w-2xl mx-4">
      <!-- Header -->
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          ...
          <!-- Camera Selection -->
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Camera Source</label>
            <select
              :value="selectedCamera"
              @change="emit('camera-change', ($event.target as HTMLSelectElement).value)"
        ...
        <div class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Microphone Source</label>
            <select
              :value="selectedMicrophone"
              @change="emit('microphone-change', ($event.target as HTMLSelectElement).value)"
              class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            >
          ...
          <!-- Audio Level Meter -->
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Audio Level</label>
              ...
              <div class="flex items-center space-x-3">
                ...
                <div class="flex-1 bg-gray-600 rounded-full h-3 overflow-hidden">
                  <div
                    ...
                    :class="audioLevel > 70 ? 'bg-red-500' : audioLevel > 40 ? 'bg-yellow-500' : 'bg-green-500'"
                    :style="{ width: `${audioLevel}%` }"
                  ></div>
                </div>
                <span class="text-sm text-gray-400 w-12">{{ Math.round(audioLevel) }}%</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-700 rounded-xl p-4">
            ...
            <p class="text-sm text-gray-400 mb-3">Speak into your microphone to test the audio level.</p>
